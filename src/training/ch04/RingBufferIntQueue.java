...
public class RingBufferIntQueue {
    ...
    //큐에 데이터를 인큐
    public int enque(int x) throws OverflowIntQueueExcption{
        if (num >= capacity){
    ...
    public int search(int y) throws EmptyIntQueueException{
        if (num <= 0){
            throw new EmptyIntQueueException();
        }
        else {
            for (int i = front; i <= num; i++){
                if (que[(i + front) % capacity] == y) {
//                    int j = que[(i + front) % capacity];
                    return i + 1;
                }
            }
        }
